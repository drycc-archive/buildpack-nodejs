...
func TestResolveYarn(t *testing.T) {
	...
	for _, c := range cases {
		...
		if assert.Nil(t, err) {
			assert.True(t, result.matched)
			assert.Equal(t, result.release.version.String(), c.output)
			assert.Equal(t, result.release.url, fmt.Sprintf("https://buildpacks.drycc.cc/nodejs/yarn/release/yarn-v%s.tar.gz", c.output))
		}
	}
}
	...
	for _, platform := range platforms {
		...
		if assert.Nil(t, err) {
			...
			if platform == "linux-x64" {
				assert.Equal(t, result.release.url, "https://buildpacks.drycc.cc/nodejs/node/release/linux-x64/node-v10.15.1-linux-x64.tar.gz")
			} else {
				assert.Equal(t, result.release.url, "https://buildpacks.drycc.cc/nodejs/node/release/darwin-x64/node-v10.15.1-darwin-x64.tar.gz")
			}
		}
		...
		if assert.Nil(t, err) {
			...
			if platform == "linux-x64" {
				assert.Equal(t, result.release.url, "https://buildpacks.drycc.cc/nodejs/node/staging/linux-x64/node-v10.15.4-linux-x64.tar.gz")
			} else {
				assert.Equal(t, result.release.url, "https://buildpacks.drycc.cc/nodejs/node/staging/darwin-x64/node-v10.15.4-darwin-x64.tar.gz")
			}
		}
