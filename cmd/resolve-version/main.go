...
func parseObject(key string) (release, error) {
	...
	if nodeRegex.MatchString(key) {
		...
		return release{
			...
			platform: match[2],
			version:  version,
			url:      fmt.Sprintf("https://buildpacks.drycc.cc/%s/node/%s/%s/node-v%s-%s.tar.gz", "nodejs", match[1], match[2], match[3], match[2]),
		}, nil
	}
	...
	if yarnRegex.MatchString(key) {
		...
		return release{
			...
			stage:    match[1],
			platform: "",
			url:      fmt.Sprintf("https://buildpacks.drycc.cc/nodejs/yarn/release/yarn-v%s.tar.gz", version),
			version:  version,
		}, nil
	}
